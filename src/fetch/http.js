...
class Http {
  ...
  request(options) {
    ...
    const successStream = fetchStream.pipe(
      ...
      mergeMap(async response => ({ data: await response.json(), status: response.status }))
    )
    const failStream = fetchStream.pipe(
      filter(response => !response.ok),
      mergeMap(async response => Promise.reject({ error: await response.json(), status: response.status }))
    )
    const mergedStream = merge(successStream, failStream)
      .pipe(
        takeUntil(this.cancelRequests.pipe(
